<template>
				...
				<el-table-column fixed="right" label="操作">
					<template #default="scope">
						...
						<div><el-button type="danger" size="small"
								@click="del_sdata(scope.row, scope.$index)">删除</el-button>
						</div>
					</template>
				</el-table-column>
			</el-table>
	...
	<!-- 这是抽屉层 -->
	<el-drawer v-model="drawer" title="GoPower参数设置" :size="drawer_size">
		<!-- 设置表单 -->
		<el-form :label-position="'top'" label-width="100px" style="max-width: 460px">
			...
			<el-form-item label="子网掩码（或掩码位元数）">
				<el-input v-model="s_data.mask" />
			</el-form-item>
...
function startup(index) {
	...
	params.append('broadcast', info.broadcast);
